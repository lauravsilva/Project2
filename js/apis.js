...
(function () {
    window.movieDatabase = {
        ...
        displayMovieResults: function (id, page) {
            ...
            movieDatabase.call(id, page, function (resp) {
        ...
        createRequest: function (requestData, callback, parseAsXml) {
            ...
            httpReq.onreadystatechange = function () {
                ...
                if (httpReq.status !== 200) {
                    document.querySelector("#dynamicContentNetflix").innerHTML = "<p class=\"red-text\"><i class=\"mdi-navigation-close\"></i>Boo! That movie/show is not on Netflix :(</p>";
                    throw new Error("Boo! That movie/show is not on Netflix");
                }
                callback(parseAsXml ? new DOMParser().parseFromString(httpReq.responseText, "text/xml") : JSON.parse(httpReq.responseText));
            };
            httpReq.send();
        }
    }
